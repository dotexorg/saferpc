...
| Method            | Effect                                  | Notes                                                                                                                                                                                                                                                                                    |
| ----------------- | --------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `.use(mw)`        | Adds middleware that can extend context | `mw` must return/await `next()` (called exactly once). `next(extra)` merges `extra` into ctx — and its type flows into every downstream step. The runtime requires the call before middleware completion, and the `next()` promise must have settled by then — a fire-and-forget `next()` yields `MIDDLEWARE`. |
...
The middleware must call `next(...)` **exactly once** before its returned promise settles, and the promise returned by `next()` must itself have settled by that point — i.e. `next()` must be awaited or returned. Calling `next()` twice throws `RPCError("MIDDLEWARE", ...)`; so does passing a non-object `extra`. Completing without calling `next()` at all also throws `MIDDLEWARE` — otherwise the handler would be silently skipped while the caller still observed a success. Likewise, completing while the `next()` promise is still pending (fire-and-forget) throws `MIDDLEWARE` — otherwise the caller would receive the middleware's own value while the handler outcome is silently dropped. A late call after completion is ignored. Awaiting `next()` and returning a different (e.g. transformed or fallback) value is supported, including catching a downstream rejection.
